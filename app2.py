...
with c2:
    if st.button("🎯 Top topics by frequency"):
        quick_query = f"What topics appear most in past {subject} papers?"
with c3:
    if st.button("📋 Important units"):
        quick_query = f"What are the most important units in {subject}?"
with c4:
    if st.button("⏱ 2-hour plan"):
        quick_query = f"Give me a 2-hour study plan for {subject} with {exam_days} days left."
...
if ask_btn and query:
    if not is_kb_ready():
        ...
        with st.spinner("Thinking..."):
            response = ask_focusdesk(query, exam_days=exam_days, subject=subject)
        st.markdown("---")
